...
function loadStudents() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const { data: teams, error: teamError } = yield window.supabase
                .from("Teams")
                .select("*");
            if (teamError)
                throw teamError;
            allTeams = teams || [];
            // Fetch all students
            const { data: students, error } = yield window.supabase
                .from("members")
                .select("*")
                .order("name");
            if (error)
...
function getStudentTeamName(student) {
    if (!student.admissionNumber)
        return null;
    const adm = String(student.admissionNumber);
    const team = allTeams.find(t => Array.isArray(t.members) &&
...
function updateStats() {
    ...
    document.getElementById("assignedStudents").textContent = String(assignedStudents);
    document.getElementById("unassignedStudents").textContent = String(unassignedStudents);
    document.getElementById("captainCount").textContent = String(captainCount);
}
function filterStudents() {
    ...
    filteredStudents = allStudents.filter(student => {
        ...
        const matchesSearch = !searchTerm ||
            ...
            ((_a = student.email) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes(searchTerm));
...
function editStudent(studentId) {
    ...
    document.getElementById("admissionNumber").value = String(student.admissionNumber || "");

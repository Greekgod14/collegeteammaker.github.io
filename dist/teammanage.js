...
let captainChanges = new Map(); // true = remove, false = add
...
function setupEventListeners() {
    ...
    if (createTeamSearch) {
        createTeamSearch.addEventListener("input", function (e) {
            filterAllStudents(e.target.value);
        });
    }
    ...
    const assignTeamSearch = document.getElementById("assignTeamSearch");
    if (assignTeamSearch) {
        assignTeamSearch.addEventListener("input", function (e) {
            filterAssignTeams(e.target.value);
        });
    }
    ...
    const addCaptainSearch = document.getElementById("addCaptainSearch");
    if (addCaptainSearch) {
        addCaptainSearch.addEventListener("input", function (e) {
            filterAvailableCaptains(e.target.value);
        });
    }
...
function loadTeamsAndStudents() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            allTeams = teams
                ? teams.map((team) => (Object.assign({ id: team.id }, team)))
                : [];
            const { data: students, error: studentsError } = yield supabase
                .from("members")
                .select("*");
            if (studentsError) {
                throw studentsError;
            ...
            allStudents = students
                ? students.map((student) => (Object.assign({ id: student.id }, student)))
                : [];
            displayTeams();
        ...
        <div class="team-card" data-team-id="${team.id}">
          ...
          <div class="team-card-body">
            ...
            <div class="team-card-actions">
              ...
              <button class="btn btn-sm btn-danger" onclick="deleteTeam('${team.id}')" ${isLocked ? 'disabled style="opacity: 0.5;"' : ""}>Delete</button>
            </div>
          </div>
        </div>
      `;
...
function filterTeams(query) {
    ...
    teamCards.forEach((card) => {
        var _a, _b;
        const teamName = ((_b = (_a = card.querySelector(".team-name")) === null || _a === void 0 ? void 0 : _a.textContent) === null || _b === void 0 ? void 0 : _b.toLowerCase()) || "";
...
function openTeamManagement(teamId) {
    ...
    if (teamManagementTitle) {
        teamManagementTitle.textContent = `Manage ${currentEditingTeam.name || `Team ${currentEditingTeam.id}`}`;
    }
    updateLockButtonState();
    const currentTeamHeader = document.getElementById("currentTeamHeader");
    if (currentTeamHeader) {
        currentTeamHeader.innerHTML = `
      <input type="text" id="editingTeamName" value="${currentEditingTeam.name || `Team ${currentEditingTeam.id}`}" class="team-name-input">
    `;
    }
            ...
            <div class="member-item ${isCaptain ? "captain-highlight" : ""}">
              <div class="member-info">
                ...
                <div class="member-details">
                  ${member.department || "Unknown Dept"} • ${member.gender || "?"}
                </div>
              </div>
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            if (currentEditingTeam.captain) {
                const { error: removeCaptainError } = yield supabase
                    .from("members")
                    .update({
                    isCaptain: false,
                })
            ...
            const { error: studentError } = yield supabase
                .from("members")
                .update({
                isCaptain: true,
            })
...
function removeMemberFromTeam(studentId) {
    if (!currentEditingTeam)
        return;
    const memberIdToRemove = currentEditingTeam.members && currentEditingTeam.members.length > 0
    ...
    if (currentEditingTeam.members) {
        currentEditingTeam.members = currentEditingTeam.members.filter((id) => id !== memberIdToRemove);
    }
    openTeamManagement(currentEditingTeam.id);
}
function saveTeamChangesHandler() {
    return __awaiter(this, void 0, void 0, function* () {
        if (!currentEditingTeam)
            return;
        try {
            ...
            if (previousTeam && previousTeam.members) {
                ...
                const removedMembers = previousTeam.members.filter(memberId => { var _a; return !((_a = currentEditingTeam.members) === null || _a === void 0 ? void 0 : _a.includes(memberId)); });
            ...
            for (const memberId of currentEditingTeam.members || []) {
                try {
                    const isCaptain = currentEditingTeam.captain === memberId;
                    const { error: studentUpdateError } = yield supabase
                        .from("members")
                        .update({
                        isCaptain: isCaptain,
                    })
    ...
    availableStudentsList.innerHTML = unassignedStudents
        ...
        <div class="member-item">
          <div class="member-info">
            ...
            <div class="member-details">
              ${student.department || "Unknown Dept"} • ${student.gender || "?"} • ${student.email || "No email"}
            </div>
          </div>
...
function filterAvailableStudents(query) {
    ...
    studentItems.forEach((item) => {
        var _a, _b;
        const studentName = ((_b = (_a = item.querySelector(".member-name")) === null || _a === void 0 ? void 0 : _a.textContent) === null || _b === void 0 ? void 0 : _b.toLowerCase()) || "";
        item.style.display = studentName.includes(searchTerm)
            ? ""
            : "none";
    });
}
function addMemberFromModal(studentId) {
    if (!currentEditingTeam)
        return;
    const memberId = typeof studentId === "string" ? parseInt(studentId) : studentId;
    if (!currentEditingTeam.members) {
...
function populateTeamsList() {
    ...
    assignTeamsList.innerHTML = allTeams
        ...
        return `
        <div class="member-item selectable ${isSelected ? "selected" : ""}" onclick="selectTeamForAssignment('${team.id}')">
          <div class="member-info">
            <div class="member-name">${team.name || `Team ${team.id}`}</div>
            <div class="member-details">
              Captain: ${captain ? captain.name : "N/A"} • ${(captain === null || captain === void 0 ? void 0 : captain.department) || "Unknown"} • ${(captain === null || captain === void 0 ? void 0 : captain.gender) || "?"}
            </div>
          </div>
        </div>
      `;
...
function filterAssignTeams(query) {
    const teamItems = document.querySelectorAll("#assignTeamsList .member-item");
    const searchTerm = query.toLowerCase();
    teamItems.forEach((item) => {
        var _a, _b, _c, _d;
        const teamName = ((_b = (_a = item.querySelector(".member-name")) === null || _a === void 0 ? void 0 : _a.textContent) === null || _b === void 0 ? void 0 : _b.toLowerCase()) || "";
        const captainName = ((_d = (_c = item.querySelector(".member-details")) === null || _c === void 0 ? void 0 : _c.textContent) === null || _d === void 0 ? void 0 : _d.toLowerCase()) || "";
        item.style.display =
...
function confirmAssignStudentHandler() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const { error: teamUpdateError } = yield supabase
                .from("Teams")
                .update({
...
function filterAllStudents(query) {
    const studentItems = document.querySelectorAll("#allStudentsList .member-item");
    const searchTerm = query.toLowerCase();
    studentItems.forEach((item) => {
        var _a, _b;
        const studentName = ((_b = (_a = item.querySelector(".member-name")) === null || _a === void 0 ? void 0 : _a.textContent) === null || _b === void 0 ? void 0 : _b.toLowerCase()) || "";
...
function addStudentToNewTeam(studentId) {
    ...
    const processedStudentId = typeof studentId === "string" ? parseInt(studentId) : studentId;
  ...
  `;
    ...
    if (captainSelect) {
        const alreadyExists = Array.from(captainSelect.options).some((opt) => opt.value === studentId.toString());
    ...
    if (allStudentsList) {
        ...
        if (itemsInList.length > 1) {
            const specificElement = allStudentsList.querySelector(`[data-student-id="${studentId}"]`);
            if (specificElement) {
...
function removeStudentFromNewTeam(studentId) {
    ...
    if (!student)
        return;
    const memberElement = (_a = document
        .querySelector(`[onclick="removeStudentFromNewTeam('${studentId}')"]`)) === null || _a === void 0 ? void 0 : _a.closest(".member-item");
    if (memberElement) {
    ...
    const captainSelect = document.getElementById("teamCaptainSelect");
    if (captainSelect) {
        const optionToRemove = Array.from(captainSelect.options).find((opt) => opt.value === studentId.toString());
        if (optionToRemove) {
...
function confirmCreateTeamHandler() {
    return __awaiter(this, void 0, void 0, function* () {
        const newTeamName = document.getElementById("newTeamName");
        const teamCaptainSelect = document.getElementById("teamCaptainSelect");
        const newTeamMembers = document.getElementById("newTeamMembers");
        if (!newTeamName || !teamCaptainSelect || !newTeamMembers)
            return;
        ...
        Array.from(newTeamMembers.querySelectorAll(".member-item[data-student-id]")).forEach((item) => {
            const studentId = item.getAttribute("data-student-id");
            if (studentId) {
                newTeamMembersArray.push(isNaN(Number(studentId)) ? studentId : Number(studentId));
            }
        });
        ...
        try {
            ...
            let { data: newTeam, error: insertError } = yield supabase
                .from("Teams")
                .insert(teamData)
                .select()
                .single();
            // If we get a duplicate key error, manually find the next available ID
            if ((insertError === null || insertError === void 0 ? void 0 : insertError.code) === "23505") {
                console.log("Duplicate key error detected, finding next available ID...");
                // Get all existing team IDs
                const { data: existingTeams, error: fetchError } = yield supabase
                    .from("Teams")
                    .select("id")
                ...
                const existingIds = (existingTeams === null || existingTeams === void 0 ? void 0 : existingTeams.map((team) => Number(team.id))) || [];
                let nextId = 1;
                ...
                const teamDataWithId = Object.assign({ id: nextId }, teamData);
                ({ data: newTeam, error: insertError } = yield supabase
                    .from("Teams")
            ...
            const updatePromises = newTeamMembersArray.map((studentId) => __awaiter(this, void 0, void 0, function* () {
            ...
            alert("Error creating team. Please check the console for details and try again.");
        }
    });
}
function openManageCaptainsModal() {
...
function closeManageCaptainsModal() {
    const manageCaptainsModal = document.getElementById("manageCaptainsModal");
    if (manageCaptainsModal) {
    ...
    captainsList.innerHTML = captains
        ...
        <div class="member-item">
          <div class="member-info">
            ...
            <div class="member-details">
              ${captain.department || "Unknown Dept"} • ${captain.gender || "?"} • ${captain.admissionNumber || "N/A"}
            </div>
          </div>
          <button class="btn btn-sm btn-danger" onclick="removeCaptain('${captain.id}')">Remove Captain</button>
        </div>
      `;
    })
        .join("");
...
function populateAvailableCaptainsList() {
    const availableCaptainsList = document.getElementById("availableCaptainsList");
    if (!availableCaptainsList)
        return;
    const currentCaptainIds = new Set(allStudents.filter((s) => s.isCaptain).map((s) => s.id));
    const availableStudents = allStudents.filter((student) => !currentCaptainIds.has(student.id));
    ...
    availableCaptainsList.innerHTML = availableStudents
        ...
        <div class="member-item">
          <div class="member-info">
            <div class="member-name">${student.name}</div>
            <div class="member-details">
              ${student.department || "Unknown Dept"} • ${student.gender || "?"} • ${student.admissionNumber || "N/A"}
            </div>
          </div>
          <button class="btn btn-sm btn-primary" onclick="addCaptainFromModal('${student.id}')">Add Captain</button>
        </div>
      `;
    })
        .join("");
}
function filterAvailableCaptains(query) {
    const studentItems = document.querySelectorAll("#availableCaptainsList .member-item");
    const searchTerm = query.toLowerCase();
    studentItems.forEach((item) => {
        var _a, _b;
        const studentName = ((_b = (_a = item.querySelector(".member-name")) === null || _a === void 0 ? void 0 : _a.textContent) === null || _b === void 0 ? void 0 : _b.toLowerCase()) || "";
...
function addCaptainFromModal(studentId) {
    captainChanges.set(studentId, false);
    closeAddCaptainModal();
    populateCaptainsList();
}
function saveCaptainChangesHandler() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            for (const [studentId, isRemoval] of captainChanges) {
                if (isRemoval) {
                    ...
                    const team = allTeams.find(t => t.members && t.members.some(memberId => memberId === studentId));
                    if (team) {
...
function toggleTeamLock() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const teamIndex = allTeams.findIndex((team) => team.id === currentEditingTeam.id);
            if (teamIndex !== -1) {
...
function showLockStatusMessage(message) {
...
function confirmFillTeamsHandler() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const teamsNeedingFilling = allTeams.filter((team) => !team.locked && // Only unlocked teams
                team.members &&
                team.members.length < maxStudentsPerTeam);
            if (teamsNeedingFilling.length === 0) {
                alert("No unlocked teams need filling. All teams are either locked or already have enough members.");
                closeFillTeamsModal();
                return;
            }
            // Show loading state
            const confirmButton = document.getElementById("confirmFillTeams");
            if (confirmButton) {
                confirmButton.textContent = "Filling...";
                confirmButton.disabled = true;
            }
            // Distribute unassigned students to teams
            const distributionResult = yield distributeStudentsToTeams(unassignedStudents, teamsNeedingFilling, maxStudentsPerTeam);
            // Update UI
            closeFillTeamsModal();
            loadTeamsAndStudents();
            // Show results
...
function distributeStudentsToTeams(unassignedStudents, teams, maxPerTeam) {
    return __awaiter(this, void 0, void 0, function* () {
        let assignedCount = 0;
        let teamsFilled = 0;
        ...
        while (studentIndex < availableStudents.length &&
            ...
            if (currentMemberCount < maxPerTeam) {
                try {
                    ...
                    console.error(`Error assigning student ${student.id} to team ${team.id}:`, error);
...
window.setAsCaptain = setAsCaptain;
window.removeMemberFromTeam = removeMemberFromTeam;
window.addMemberFromModal = addMemberFromModal;
window.addStudentToNewTeam = addStudentToNewTeam;
window.removeStudentFromNewTeam = removeStudentFromNewTeam;
window.selectTeamForAssignment = selectTeamForAssignment;
window.removeCaptain = removeCaptain;
window.addCaptainFromModal = addCaptainFromModal;
window.closeCreateTeamModal = closeCreateTeamModal;
window.closeAddMemberModal = closeAddMemberModal;
window.closeManageCaptainsModal = closeManageCaptainsModal;

...
async function loadTeamData(): Promise<void> {
  try {
    ...
    showError(
      "Failed to load team data: " + (error?.message || "Network error")
    );
  } finally {
    hideLoading();
  }
...
function displayTeamData(): void {
  ...
  const nameButtons = document.getElementById(
    "teamNameButtons"
  ) as HTMLElement | null;
...
function displayTeamMembers(): void {
  ...
  grid.innerHTML = teamMembers
    .map((member) => {
      ...
      const isSelf =
        currentUser && member.admissionNumber === currentUser.admissionNumber;
            ...
            <div class="member-card ${isCaptain ? "captain" : ""}">
                <div class="member-info">
                    ...
                    <div class="member-role">
                        ${member.department || "Unknown"} • Section ${
        member.section ?? "N/A"
      }
                    </div>
                    <div class="member-id">
...
// ---------- Load Team Mentor ----------
async function loadTeamMentor(): Promise<void> {
  ...
  currentMentor =
    mentors.find((mentor: Mentor) => {
      if (!mentor.teams || !Array.isArray(mentor.teams)) return false;

      return mentor.teams.some(
        (t) => String(t) === teamIdStr || Number(t) === teamIdNum
      );
    }) || null;
  ...
  container.innerHTML = `
    <div class="mentor-name">
      ${currentMentor.name ?? "Unnamed Faculty"}
    </div>
  `;
}
